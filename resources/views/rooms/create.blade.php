...
<div class="mb-4">
    ...
    <h3 class="fw-bold mt-2 mb-1">Add New Room</h3>
    <p class="text-muted mb-0">Register a new room into the system.</p>
</div>
        ...
        <div class="card card-custom p-4">
            <form method="POST" action="{{ route('rooms.store') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="room_code" id="room_code" value="{{ old('room_code', $nextCode) }}" class="form-control @error('room_code') is-invalid @enderror" required>
                    @error('room_code')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" placeholder="e.g. Executive Meeting Room" required>
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity', 20) }}" min="1" class="form-control @error('capacity') is-invalid @enderror" required>
                    @error('capacity')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
